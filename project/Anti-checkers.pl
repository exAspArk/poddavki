...
%!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! GLOSSARY !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!%
% checkers       - шашки (амер.)
% anti-checkers  - поддавка (also suicide checkers, giveaway checkers)
% computer       - компьютер
% player         - игрок
% checker        - шашка
% king           - дамка
...
% проверка на то, что все клетки от (X1, Y1) в направлении (Sx1, Sy1) пустые
...
% определяем (X2, Y2) куда сходить из (X1, Y1), чтобы фигура игрока съела эту дамку начиная поиск со смещения = 1 во все стороны, увеличивая его, пока не найдем, либо пока не выйдем за область поля
computer_king_next_step_player_can_kill(X1, Y1, X2, Y2, N) :- 
	...
	empty(X2, Y2), 
	retract(computer_king(X1, Y1)),	% смещаем дамку
	assert(computer_king(X2, Y2)),
	(player_need_kill ;  			% фигура игрока может съесть дамку
	(retract(computer_king(X2, Y2)), assert(computer_king(X1, Y1)), fail)), % если нет, то возвращаем дамку на место и продолжаем поиск
	!.
computer_king_next_step_player_can_kill(X1, Y1, X2, Y2, N):-
	computer_king(X1, Y1),
	X2 is X1 + N,
...
% съесть дамкой из (X1, Y1) перейдя в (X2, Y2) фигуру (Gx, Gy), 
% где Killed - список уже убитых (чтобы не убивать их снова), From - список, где была дамка (чтобы не возвращаться), Remove - флаг,на удаление всех фигур
computer_king_need_kill(X1, Y1, X2, Y2, Gx, Gy, Killed, From, Remove):-
    ...
    Sy1 is sign(Dy),
    add([X1, Y1], From, FromNew),
	all_clear(X1, Y1, Sx1, Sy1, Gx, Gy),% все клетки до цели (Gx, Gy) от нашей дамки (X1, Y1) пустые
    computer_king_next_cell(Sx1, Sy1, X2, Y2, Gx, Gy, Killed, FromNew, Remove), % переход на следующую клетку за съеденой фигурой, чтобы проверить, можно ли съесть еще фигуры, если прыгнуть дамкой дальше
    !.
...
computer_king_next_cell(Sx1, Sy1, X2, Y2, Gx, Gy, Killed, From, Remove):-
	...
	empty(X, Y),					% удостоверяемся, что там пусто
    (
        (                           % если нашли элемент, который можно съесть, добавляем его в список KilledNew
            (player_checker(Gx, Gy) ; player_king(Gx, Gy)), 
            copy(From, FromNew),
            add([Gx, Gy], Killed, KilledNew)
        ) ; 
        (                           % если в новой клетке пусто, сохраняем ее в FromNew, чтобы потом больше не возвращаться
            copy(Killed, KilledNew), 
            add([Gx, Gy], From, FromNew)
        )
    ),
	(
	    (
            computer_king_can_kill(X, Y, X2, Y2, KilledNew, FromNew, Remove) ; % компьютер может съесть еще фигуры противника из новой позиции
			computer_king_next_cell(Sx1, Sy1, X2, Y2, X, Y, KilledNew, FromNew, Remove) % если нет, то переходим на следующую клетку в том же наприавлении
    	) ;
    	(                           % если некуда шагать дальше (стоит фигура которую нельзя съесть или кончилось поле)
        	computer_figure_in(KilledNew),
        	X2 is X,				% возвращаем последние координаты, где находится наша фигура после всех "приключений"
        	Y2 is Y,
        	((Remove = 1, computer_remove_all(KilledNew)) ; !) % удаляем все фигуры из БД, если флаг Remove = 1
    	)
	), !.
...
computer_king_can_kill(X, Y, X2, Y2, Killed, From, Remove):-
    (player_checker(Gx, Gy) ; player_king(Gx, Gy)),
    not(is_member([Gx, Gy], Killed)),	% если фигура, которую можно съесть еще не в списке Killed    
	computer_king_need_kill(X, Y, X1, Y1, Gx, Gy, Killed, From, Remove), % съедаем все возможные фигуры и определяем, куда попадает дамка после съедения
	X2 is X1,
	Y2 is Y1,
	!.	
...
computer_move:-
	...
	computer_checker_need_kill(X1, Y1, X2, Y2, Gx1, Gy1),	  % шашка на клетке (X1, Y1) может съесть фигуру на клетке (Gx1, Gy1), перейдя на (X2, Y2)
	computer_checker_can_continue(X2, Y2, X, Y, [[Gx1, Gy1]]),% шашка может продолжить съедать из (X2, Y2), перейдя на (X, Y)
	retract(computer_checker(X1, Y1)),	% удаление данных о предыдущем положении шашки компьютера
	assert(computer_checker(X, Y)),		% запись данных о новом положении шашки компьютера
	computer_try_to_get_king(X, Y),		% проверка на дамку
	!.		  	

% дамка убивает
computer_move:-
	computer_king(X1, Y1),			% на (X1, Y1) дамка
	computer_king_need_kill(X1, Y1, X2, Y2, Gx, Gy, [], [], 1),% компьютер ходит из (X1, Y1) в (X2, Y2), съедая все возможные фигуры
	retract(computer_king(X1, Y1)),	% удаление данных о предыдущем положении дамки компьютера
	assert(computer_king(X2, Y2)),	% запись данных о новом положении дамки компьютера
	!.

% дамка умно ходит
computer_move:-
	computer_king(X1, Y1),    
	computer_king_next_step_player_can_kill(X1, Y1, X2, Y2, 1), % определяем (X2, Y2) куда сходить из (X1, Y1), чтобы фигура игрока съела эту дамку начиная поиск со смещения = 1 во все стороны
	!.

% шашка ходит из (X1, Y1) в (X2, Y2) так, что игроку придется съесть ее
computer_move:-
	computer_checker(X1, Y1),			    % шашка компьютера на (Х1, У1)
	computer_can_move(X1, Y1, X2, Y2),	    % вариант (X2, Y2), куда может сходить шашка из (X1, Y1)
	empty(X2, Y2),						    % клетка (X2, Y2) пуста
	computer_checker_next_step_player_can_kill(X1, Y1, X2, Y2), % нахождение такой (X2, Y2), в которой игроку придется съесть шашку
	computer_try_to_get_king(X2, Y2), 		% проверка на то, не стала ли шашка дамкой
	!.

% шашка умно ходит, так как пока нету шанса, чтобы после хода компьютера игрок съел фигуру 
computer_move:-
    computer_checker_smart_go_first(6, Y, X2, Y2, X), % нахождение шашки (X, Y), которая находится как можно ниже, которая может сходить на (X2, Y2)
...
computer_move:-
    ...
    Sx1 is sign(Dx),                	% стороны смещения в направлении фигуры, которую надо съесть
    Sy1 is sign(Dy),
	all_clear(X1, Y1, Sx1, Sy1, X2, Y2),% все клетки до фигуры, которую надо съест, пустые
	empty(X2, Y2),
	retract(computer_king(X1, Y1)),
	assert(computer_king(X2, Y2)).
...
computer_checker_smart_go_first(X, Y, X2, Y2, G):-
    (
        (
			...
			empty(X2, Y2),
			not(player_next_step(X2, Y2)),	% если нет фигур противника, находящихся в опасной зоне (расстояние в 1 шаг) от (X2, Y2)
			G is X
		) ;
    	(X1 is X - 1, X1 >= 0, computer_checker_smart_go_first(X1, Y, X2, Y2, G))
    ), !.

% нет фигур игрока, находящихся в опасной зоне (расстояние в 1 шаг) от (X2, Y2)
